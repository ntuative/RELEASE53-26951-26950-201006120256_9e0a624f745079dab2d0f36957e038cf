package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      private var var_1576:Rectangle;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_630.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_313.x = _loc3_.dirty.x;
               var_313.y = _loc3_.dirty.y;
               var_313.width = _loc3_.dirty.width;
               var_313.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_313);
            }
            _loc4_++;
         }
      }
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1575,var_1576))
            {
               if(param2.render(param1,var_1575,var_1576))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_767);
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_313.x = param2.x - param1.x;
            var_313.y = param2.y - param1.y;
            var_313.width = param2.width;
            var_313.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_36))
            {
               while(param1.testParamFlag(WindowParam.const_36))
               {
                  ...
                  var_313.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_313,WindowRedrawFlag.const_273);
