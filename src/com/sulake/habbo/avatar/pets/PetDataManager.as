package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private var var_126:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(var_447.getValue(_loc4_) == null)
               {
                  var_447.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(var_1133.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(var_1132.indexOf(_loc6_) == -1)
               {
                  ...
                  var_256.push(_loc8_);
                  var_1132.push(_loc6_);
                  if(param3 != null && var_730.indexOf(param3) == -1)
                  {
                     var_730.push(param3);
                  }
               }
      ...
      private function onLoaderComplete(param1:Event) : void
      {
         ...
         var_1133.push(_loc2_.url);
         var _loc3_:int = var_256.indexOf(_loc2_);
         if(_loc3_ >= 0)
         {
      ...
      {
         var _loc3_:IPetData = var_447.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_730.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      {
         var _loc4_:PetData = var_447.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_447.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_447.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.MeMenuWidget.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
