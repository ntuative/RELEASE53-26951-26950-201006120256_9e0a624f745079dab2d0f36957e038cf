package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1746:int = 3;
      
      public const const_1607:int = 2;
      ...
      private var var_2222:MarketplaceItemStats;
      ...
      {
         if(!var_160 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_160.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1673,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_2159 || param1.furniTypeId != var_2162)
         {
            return;
         }
         var_2222 = param1;
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = var_209.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = var_160.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = var_160.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_160.add(parser.offerId,updateItem);
            }
            var_160.remove(parser.requestedOfferId);
            showConfirmation(const_1607,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_209.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_209.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_422)
            {
               var_209.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_1990 = _loc3_.creditsWaiting;
         var_209 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_209.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_160.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_2224,var_2221,var_1944,var_2223);
      }
